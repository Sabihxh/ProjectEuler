...
def is_prime(a_num):
    ...
    if a_num == 2: return True
    if not a_num & 1: return False #return false for even numbers

    limit = int(sqrt(a_num)) + 2
    for x in range(3, limit, 2):
        if a_num % x == 0: return False
...
def solution(prime_limit, main_loop_limit): #increase these limits until the function prints out a number
