    ...
    for x in range(11, loop_limit - 1):
        ...
        for y in range(x + 1, loop_limit):
            ...
            if len(common_digit) == 1:
                ...
                wrongly_simplified = simplify_fraction(
                    int(num_simplified_wrong), int(den_simplified_wrong)
                )
                simplified_xy_fraction = Fraction(x, y)

                # Matching numerator and denominator of two fractions
                if (
                    simplified_xy_fraction.numerator == wrongly_simplified[0]
                    and simplified_xy_fraction.denominator == wrongly_simplified[1]
                ):
                    list_of_fractions.append(simplified_xy_fraction)

    product_of_fractions = reduce(
        (lambda x, y: x * y), [item for item in list_of_fractions]
    )
